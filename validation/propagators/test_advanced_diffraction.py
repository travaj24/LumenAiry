...
def t_mutual_coherence_diagonal():
    ...
    for angle in [0, 0.005, -0.005, 0.003, -0.003]:
        E, _, _ = la.create_tilted_plane_wave(N, dx, lam, angle_y=angle)
        fields.append(E)
    Gamma, _ = la.mutual_coherence(fields, dx)
...
def t_extended_source_smoothing():
    ...
    I_narrow = la.koehler_image(E_obj, pres, lam, dx,
                                condenser_NA=0.01, n_source_points=3)
    I_wide = la.koehler_image(E_obj, pres, lam, dx,
...
def t_koehler_imaging_finite_intensity():
    ...
    I = la.koehler_image(E, pres, lam, dx,
