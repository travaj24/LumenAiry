...
def t_asm_vs_fresnel():
    ...
    a = N * dx / 2
    F_num = a**2 / (lam * z)
    E_in, _, _ = la.create_gaussian_beam(N, dx, lam, w0=(100e-6) * np.sqrt(2))
...
def t_rs_vs_asm_near_field():
    ...
    E_in, _, _ = la.create_gaussian_beam(N, dx, lam, w0=(20e-6) * np.sqrt(2))
    E_asm = la.angular_spectrum_propagate(E_in, z, lam, dx)
...
def t_rs_h_cache_idempotent():
    ...
    E_in, _, _ = la.create_gaussian_beam(N, dx, lam, w0=(30e-6) * np.sqrt(2))
...
def t_sas_cache_idempotent():
    ...
    E_in, _, _ = la.create_gaussian_beam(N, dx, lam, w0=(80e-6) * np.sqrt(2))
...
def t_asm_mft_cache_idempotent():
    ...
    E_in, _, _ = la.create_gaussian_beam(N, dx_in, lam, w0=(30e-6) * np.sqrt(2))
...
def t_zero_distance_propagation_is_identity():
    ...
    E_in, _, _ = la.create_gaussian_beam(N, dx, lam, w0=(40e-6) * np.sqrt(2))
