...
geom=_homog_geom_cache(sol)
W,V,lam=_homog_region_modes(geom,1.0+0j)
no=4
ox=np.arange(-no,no+1)
oy=ox
order_x=np.tile(ox,len(oy))
order_y=np.repeat(oy,len(ox))
P1,P2=_far_projector_2d(sol.bx,sol.by,ox,oy,a0x,0.0)
qq=sol.q*sol.q
H=_pmm2d_project_orders(P1,P2,W,qq)
Nfo=len(order_x)
bx,by=sol.bx,sol.by
xs=np.linspace(0.005,0.995,120)*P
y0=0.137*P
def ev(basis,sets,xarr):
    seg=np.clip((xarr/basis.h).astype(int),0,basis.N-1)
    u=2.0*(xarr-basis.xb[seg])/basis.h-1.0
    S=np.array(sets)
    out=np.zeros((S.shape[0],len(xarr)),dtype=complex)
    for i,(s,uu) in enumerate(zip(seg,u)):
        Vv,_=_modleg_value_deriv(basis.M,np.array([uu]))
        out[:,i]=S[:,s,:]@Vv[:,0]
    return out
fx=ev(bx,bx.B,xs)
fy=ev(by,by.Btilde,np.array([y0]))[:,0]
qx=bx.dim
for mtarget in (0,1,-1):
    sel=((order_x==mtarget)&(order_y==0)).astype(complex)
    rhs=np.concatenate([sel,0.0*sel])
    cinc=_guarded_lstsq(H,rhs,"probe")
    e1=(W@cinc)[:qq]
    coef=(e1.reshape(by.dim,qx)*fy[:,None]).sum(axis=0)
    E1x=coef@fx
