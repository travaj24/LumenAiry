...
o1 = np.asarray(o1); idx = {int(m): j for j, m in enumerate(o1)}
o2, R2, T2, J2 = pc.solve_slab(P, P, cell4, 1.5, 1.0, DEP, WL, M=8,
                               theta=th, phi=0.0, candidate="eform")
o2 = np.asarray(o2); sel = o2[:,1] == 0
ox = o2[sel,0]; Rp = np.asarray(R2)[:,sel]; Tp = np.asarray(T2)[:,sel]
os3, Rs, Ts = pmm_efficiency_2d_staggered(P, P, cell2, 1.5, 1.0, DEP, WL,
                                          degree=8, n_orders=5,
                                          polarization="te", theta=th, phi=0.0)
os3 = np.asarray(os3); ss = os3[:,1]==0; oxs = os3[ss,0]; Rsh = np.asarray(Rs)[ss]
print("  m  |  oracle R(+m)   oracle R(-m) | PROBE(eform) row1  SHIPPED te")
for m in (-2,-1,0,1,2):
    j = int(np.where(ox==m)[0][0]); k = int(np.where(oxs==m)[0][0])
