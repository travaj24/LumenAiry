...
def run(N, M, th, ph, nsl=None, walk=-1.0):
    """``walk`` is the DIRECTION the staircase marches the pillar, as a multiple
    of the prototype's internal ``t``.  It is SCANNED (+1 and -1), not assumed:
    the prototype's internal ``t`` is the NEGATIVE of the public / hybrid slant
    convention (M4, M3), so a staircase built with ``walk = +1`` marches the
    pillar the WRONG way and is the negative control."""
    if nsl is None:
        ...
        h = DEPTH / nsl
        layers = [{"thickness": h,
                   "cell": cell_at(N, walk * TX * DEPTH * i / nsl),
...
PLAN = (("normal", 0.0, 0.0, 8, 3, (1, 2, 4), (-1.0, +1.0)),
        ("conical20_35", np.deg2rad(20), np.deg2rad(35), 8, 3, (1, 2, 4),
         (-1.0, +1.0)),
        ("normal", 0.0, 0.0, 12, 3, (1, 2, 3, 6), (-1.0,)))

for mount, th, ph, N, M, ns, walks in PLAN:
    h = PX / N
    met, tm, cm = run(N, M, th, ph)
    print(f"\n[{mount}/Nx{N}_M{M}]  h = {h:.3f} ({h / 0.3:.2f} pillar widths)"
          f"  metric layer {tm:6.1f}s  |R+T-1| {cm:.2e}", flush=True)
    for walk in walks:
        key = f"{mount}/Nx{N}_M{M}/walk{'+' if walk > 0 else '-'}"
        rows = {"h": h, "step_over_pillar_width": h / 0.3, "walk": walk,
                "metric": {"t_s": tm, "closure": cm,
                           "dim": int(4 * (N * (M - 1)) ** 2)}}
        print(f"  -- staircase marching {'WITH' if walk < 0 else 'AGAINST'} "
              f"the prototype's slant (walk = {walk:+.0f})", flush=True)
        for n in ns:
            v, t, clo = run(N, M, th, ph, nsl=n, walk=walk)
            rows[f"n{n}"] = {"vs_metric": dd(v, met), "t_s": t,
                             "closure": clo,
                             "lateral_step": TX * DEPTH / n,
                             "cost_ratio": t / tm}
            print(f"     n={n:2d}  step {TX * DEPTH / n:.3f} "
