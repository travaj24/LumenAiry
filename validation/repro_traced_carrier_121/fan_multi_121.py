...
# TILE (per-frame readout window), NW (workers), ORDERS_ONLY=1 to stop after
# the design-side order table.
#
...
res = la.propagate_traced_carrier_chain_multi(
    ...
    progress=lambda k, K, nm: print(f"  [{k + 1}/{K}] {nm}", flush=True))
...
print("     on axis = 0.007 um of blur on a 3.5 um FWHM), not the fine")
