# C12 -- the design-121 per-order table with the PHYSICS PREDICTOR.
#
...
#
#   'C12'    ``DECENTRED_FIT_PREDICTOR = True``: the branch is chosen by the
#            closed-form crossover u* derived from the lens's own spectral
#            tail and the disc-inflation law (niche C12)
#   'C11'    ``DECENTRED_FIT_ARBITER = True``: both ray
...
#   'default' NOTHING patched -- whatever the module ships.  BOTH flags ship
#            OFF, so this arm must be BYTE-IDENTICAL to 'v532', which is what
#            makes "flag-off is a no-op" a measurement on the DESIGN rather
#            than an argument about the code.
#
# SCRIPT-SIDE ONLY -- no library file is edited by this runner.
#
# usage:  ORDERS='0,0 -1,0 -2,0 -3,0 -4,0 -4,-2' python c12_gate_arms_121.py
import hashlib
import os
import sys
import time
...
ARMS = [('default', []),
        ('C12', [(_LT, 'DECENTRED_FIT_PREDICTOR', True),
                 (_LT, 'DECENTRED_FIT_ARBITER', False)]),
        ('C11', [(_LT, 'DECENTRED_FIT_PREDICTOR', False),
                 (_LT, 'DECENTRED_FIT_ARBITER', True)]),
        ('v532', [(_LT, 'DECENTRED_FIT_PREDICTOR', False),
                  (_LT, 'DECENTRED_FIT_ARBITER', False)])]
