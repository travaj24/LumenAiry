...
def t_symbols_in_all():
    missing = [n for n in MUST_EXPORT if n not in la.__all__]
    return len(missing) == 0, \
...
def t_progress_scaler_2arg():
    ...
    scaler = la.ProgressScaler(parent, 'wrap', 0.2, 0.8)
    scaler(0.5, 'inline')
...
def t_coating_with_real_lens():
    ...
    n_baf10 = la.get_glass_index('N-BAF10', lam)
    layers = la.quarter_wave_ar(n_baf10, lam)
    R, _, _ = la.coating_reflectance(layers, [lam], n_substrate=n_baf10)
...
def t_interferogram_of_real_lens():
    ...
    for i in range(N):
        opd_2d[i, :] = np.interp((np.arange(N)-N//2)*dx, x_w, opl,
                                 left=0, right=0)
    fringe = la.simulate_interferogram(opd_2d, lam, tilt_x=500, dx=dx)
...
def t_fiber_to_detector():
    ...
    img, _, _ = la.apply_detector(E_focus, dx, pixel_pitch=20e-6,
...
def t_should_split():
    total = 16384**2 * 16
    result = la.should_split(total, available=2e9, safety=0.5)
...
def t_pick_batch_size():
    bs = la.pick_batch_size(100, 100e6, available=2e9, safety=0.5)
...
def t_plot_through_focus():
    from lumenairy.analysis.through_focus import ThroughFocusResult
    ...
    fig, _ = la.plot_through_focus(scan)
    import matplotlib.pyplot as plt
    plt.close(fig)
    return fig is not None, 'fig created'
...
def t_singlet_paraxial_focus_matches_wave_peak_position():
    ...
    I_pre = float(np.abs(
        la.angular_spectrum_propagate(E_lens, bfl - 2e-3, lam, dx)[N//2, N//2])**2)
    I_post = float(np.abs(
        la.angular_spectrum_propagate(E_lens, bfl + 2e-3, lam, dx)[N//2, N//2])**2)
...
def t_glass_dispersion_normal_for_N_BK7():
    ...
    import lumenairy as la
    n_F = float(la.get_glass_index('N-BK7', 0.486e-6))
    n_d = float(la.get_glass_index('N-BK7', 0.587e-6))
    n_C = float(la.get_glass_index('N-BK7', 0.656e-6))
