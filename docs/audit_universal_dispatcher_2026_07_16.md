...
## Follow-up — v5.24.2 (2026-07-17)

Completed the items deferred / flagged in the v5.24.1 resolution above:

- **F3 -> FULL FIX.** `mem_budget_mb` now chunks the POSITION lattice (`nq_chunk`) as well as the momenta, so a large aperture runs within the budget as an additive sum over lattice chunks (no OOM) -- the v5.24.1 guard's separable-fallback / clear-error is superseded by `_resolve_nq_chunk`. Bit-close to the un-chunked full swarm (`~2e-14`) across separable / direct / coefficient-pruning / vector; coefficient pruning stays exact via a global-per-momentum-peak pre-pass. (The 24 mm aperture is still impractically slow -- the ray trace scales with `Nq` -- but it no longer OOMs.)
- **Secondary note 1 (two dispatchers).** Documented `apply_real_lens_universal` as canonical and `apply_real_lens_auto` as the beamlet-only GBD/FGA subset; both `auto` members handle diverging beams (no bare `traced`), so `auto` needs no collimation split.
- **Secondary note 2 (caustic keys on the output plane).** Documented that a split-step caller at `output_plane_distance=0` never triggers the `fga` branch, with the workaround.
- **FGA diverging-beam accuracy (found during F1 validation).** Confirmed the ~0.93 fidelity is the FROZEN-approximation limit, NOT edge truncation (2x grid: output edge-energy `1.3e-3 -> 1.4e-9`, fidelity `0.926 -> 0.928` -- unchanged). Documented the limitation in the FGA module docstring; no code fix (inherent to the non-spreading frozen beamlet) -- the dispatcher already routes such beams to phase_screen.
