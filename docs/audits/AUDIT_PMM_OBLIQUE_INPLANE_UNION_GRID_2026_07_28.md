...
| **R-6** | **P2** | **Per-layer element grids with interface projection (§8.2)** — removes the collision class outright, retires `min_feature` as an accuracy knob, lets `n_slice` scale, and is plausibly a large net **speed-up**. ~1–3 weeks. **The recommended structural fix.** | **implemented (v1, opt-in)** -- `PMMStack(layer_grids='per-layer')`, measured 91%->5-6% degree spread at the DEFAULT `min_feature` and 17.8x faster; see AUDIT_PMM_PER_LAYER_GRIDS_IMPL_2026_07_28.md |
