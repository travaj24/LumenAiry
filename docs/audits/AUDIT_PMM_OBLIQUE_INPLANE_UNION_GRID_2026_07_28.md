...
| **R-6** | **P2** | **Per-layer element grids with interface projection (§8.2)** — removes the collision class outright, ~~retires `min_feature` as an accuracy knob~~ (**CORRECTION S-4, M4 2026-08-04**: it does not — the window IS a `2*halfwidth+1`-layer union and the snap runs on it, `_core.py:3599-3606`; `min_feature` is *dormant at the library default* and remains the intended T3-2 lever), lets `n_slice` scale, and is plausibly a large net **speed-up**. ~1–3 weeks. **The recommended structural fix.** | **implemented (v1, opt-in)** -- `PMMStack(layer_grids='per-layer')`, measured 91%->5-6% degree spread at the DEFAULT `min_feature` and 17.8x faster; see AUDIT_PMM_PER_LAYER_GRIDS_IMPL_2026_07_28.md |
