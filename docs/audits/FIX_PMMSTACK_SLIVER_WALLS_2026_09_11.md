...
| **F — the refusal cannot separate the sliver from the quasi-resonance on a real taper** | A tapered staircase carries BOTH: cross-layer collisions (ratio 157 on the M1 audit staircase, ~167 on the M2 2° coated pillar — only 1.2 decades above conjunct (a)'s bar) AND the many-interface quasi-resonance `_warn_stack_energy` was written for. When such a stack reads super-unity the refusal fires and names the sliver, which may not be the operative cause. Mitigated, not closed: the message's remedy (4) names `n_slices` / `degree` explicitly and says the refusal cannot tell the two apart. |
