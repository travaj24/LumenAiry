...
| `d3::_mux_chain_field` (1 id) | a test FAILING for the wrong reason (**not** "silently becoming vacuous" -- ROUND 2 correction, VERIFY-WP-C3 claim 12) | the caller subtracts two of these fields to show the residual-eikonal degree MOVES the multiplexed route, and on the flipped default the L2 of `E6 - E4` reads **exactly 0.0** against a bar of `> 1.0`, so the id goes RED and says the degree is INERT -- the fail-before arm of its own claim, reached for the wrong reason.  Nothing is silent about it, and calling it vacuous mis-states which way the gate breaks.  (Re-measured either side: on `'sziklas'` the same helper reads `moved = 22.552654`, the same on 49ddf4bd, so the docstring's "39.83 / 43.88" was already stale before this package and is not charged to it.)  Not a number to re-pin |
