...
| **L9** | partially fixed (see §2) | `_lens_real.py:1101-1119` (coarse grid), `:1857` / `:1860` (`_DISP_REMAP_2D_N_SIDE`, `_warn_if_remap_lattice_smooths`) + the call at `:5536`, `:612-623`/`:1190-1201`/`:1626-1637`/`:1932-1943` (Newton) | covered by the byte-identity + cache tests | cos grid at `n_coarse=4096`; image-plane mirror symmetry | pointwise cos-grid error inside the pupil **1.5e-6…3.6e-5 → 0.0** at every pad factor; Newton sweeps 24 → **13** (bitwise fixed point, output unchanged by construction; `_surface_sag_general` calls 150 → 84 = 1.79×, re-measured by VERIFY-A2 — an earlier revision of this row said "→ 2", which is where the residual reaches zero to a TOLERANCE, not where `t` stops moving in its last bits); the remap's 181-lattice smoothing now WARNS — raising the lattice measured **4.1e-03 mirror-symmetry residual at 512 against 7.9e-14 at 181** and is deferred (§6.2) |
...
VERIFY-A2 re-measured the saving: the bitwise fixed point arrives after **13**
sweeps in the two ray-map builders (3 in `_build_displaced_cos_luts`), i.e.
`_surface_sag_general` calls 150 → 84 = **1.79×**, not the ~10× quoted here
originally; and forcing the old 24 sweeps back changes no bit of any builder's
output on 2 prescriptions × 4 radii × 3 grid sizes.
