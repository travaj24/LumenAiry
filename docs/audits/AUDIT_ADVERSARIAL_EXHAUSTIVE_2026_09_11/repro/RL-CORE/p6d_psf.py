...
for nm, rx in (('plano-convex 4mm', pc()), ('doublet 4mm', dbl())):
    ap = rx['aperture_diameter']
    h0 = np.linspace(-0.995*ap/2, 0.995*ap/2, 1001)
    r = trace_meridional(rx, lam, h0)
    NA = float(np.max(np.abs(r['Lx']))); dx = 0.30*lam/NA
    N = int(2**np.ceil(np.log2(1.45*ap/dx)))
    f_est = float(np.abs(r['x'][-1]/r['Lx'][-1]))
    zs = f_est*np.linspace(0.97, 1.03, 13)
    print(f"== {nm}: N={N} dx={dx*1e6:.3f}um f~{f_est*1e3:.3f}mm ==", flush=True)
    E = np.ones((N, N), dtype=np.complex128)
    for kw in ({}, dict(seidel_correction=True)):
        Eo = apply_real_lens(E.copy(), prescription=rx, wavelength=lam, dx=dx, **kw)
        best = (-1, None)
        for z in zs:
            p = float(np.abs(asm(Eo.copy(), z, lam, dx)).max()**2)
            if p > best[0]: best = (p, z)
        print(f"   {str(kw) or 'default':<32} peak |E|^2 = {best[0]:10.2f} at z = {best[1]*1e3:8.4f} mm", flush=True)
