...
distinct over 8** on the same fixture -- intermittently, as this build does --
and across sixteen calls of the two trees the hash they settle on is the same
one, which is a byte-identity confirmation at the runner's own configuration.  This is the shipped path (the
...
v5.35.2's verify shard (py3.10 ubuntu) failed the restructured
"nothing lost" arm: its match radius for "the fix kept this pre-fix
entry" was a converged tolerance (`_VALUE_RTOL`), but a pre-fix
detector reading halts wherever that build's minimiser halts — the
shard's halted 129x the Brent floor from the converged zero, so a
*kept* mode classified as *dropped*.  Matching is now by the mode
ISOLATION radius — half the smallest inter-mode gap of the fixed
census, computed at runtime and asserted to clear the detection cell —
so basin assignment is build-free; matched entries must additionally
be converged and FD-confirmed (never checked before).  A full
classification of all 43 assertions found five that tied a pre-fix
reading to a universal constant; all five fixed, and `_absent` lost
its default tolerance so every call site must state converged vs
halted-at.  The deterministic `_stop_offset` injector reproduces both
shard failures on any build and, run against the v5.35.2 file, found
a second latent instance the shard had not yet reached.  Verified on
five environments (py3.10/3.11/3.12/3.14, three numpy/scipy stacks) —
noting that python version was never the discriminator (the wheel's
LAPACK is), which is why the injector is now a permanent rung of the
determinism ladder.  Test-file-only; the library is untouched.
