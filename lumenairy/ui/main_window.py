...
class MainWindow(QMainWindow):
    ...
    def _show_whats_new(self, *, force_show=False):
        ...
        body = (
            ...
            '<p>v5.4.0 is the largest release in the v5.x line; '
            'it closes the v5.3.2 library + GUI audits in a '
            'coordinated ship.</p>'
            '<h4>New docks</h4>'
            '<ul>'
            '<li><b>Wavefront map</b> — OPD visualisation with '
            'live optimiser hook</li>'
            '<li><b>AO closed-loop</b> — DM + WFS + leaky-'
            'integrator controller</li>'
            '<li><b>Coronagraph workflow</b> — 4-stop chain '
            'builder</li>'
            '<li><b>Operator algebra</b> — symbolic optical-'
            'system construction</li>'
            '<li><b>Thin-film coatings</b> — stack designer '
            'with 12-material registry</li>'
            '<li><b>Log viewer</b> — v5.3.2 telemetry stream</li>'
            '<li><b>Chebyshev fit</b> — freeform metrology tool</li>'
            '</ul>'
            '<h4>Library extensions</h4>'
            '<p>7 new primitives retire dock-inline workarounds: '
            '<code>make_shack_hartmann_wfs</code>, '
            '<code>zernike_decompose</code> (normalization / '
            'weighting), <code>retrace_ghost_path</code>, '
            '<code>create_*_phase_mask</code>, '
            '<code>COATING_MATERIAL_REGISTRY</code>, '
            '<code>chebyshev_fit_2d</code>.</p>'
            '<p><b>See <a href="https://github.com/travaj24/'
            'LumenAiry/blob/main/CHANGELOG.md">CHANGELOG.md</a></b> '
            'for complete history.  Press Ctrl+K for the command '
            'palette to discover new docks.</p>'
        )
