...
# Interpolating those into CODE positions of the generated script --
# ``la.GLASS_REGISTRY['{g}'] = ...``, ``print("Applying {label}
# ...")``, ``print('Running: {sys_name}')`` -- without escaping turns a
# whitespace-free ``GLAS`` token such as ``X'];<payload>;#`` into live code
...
def generate_simulation_script(
    ...
    wavelength : float or None
        ...
        (the default is not 1.31e-6; see docs/history/lumenairy.io.codegen.md).
...
def _generate_unrolled(steps, wavelength, N, dx, source_sigma,
    ...
    # --- Header ---
    # Stamp the generating library version into the
    ...
    # Upper-bound major-version-bump warning.
    ...
    # Emit the canonical ``w0`` waist (1/e^2 intensity
    ...
    for step in steps:
        ...
        if step['type'] == 'propagate':
            ...
            r = step['radius']
            # The prescription stores the mirror radius
...
def _generate_system_style(steps, wavelength, N, dx, source_sigma,
    ...
    # Same version-pin rationale as _generate_unrolled.
    from .. import __version__ as _LA_VERSION
