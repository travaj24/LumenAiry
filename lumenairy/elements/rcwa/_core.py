...
WHAT IS IN THIS FILE AND WHAT IS BESIDE IT.  ``__all__`` at the bottom lists
every name the package's ``from ._core import *`` publishes, private ones
included, so an import path here is a contract even for a leading-underscore
name.  That list is therefore NOT a module boundary, and the file is read in
SECTIONS instead, each opened by a banner comment: BLAS controls; energy and
conditioning guards; branch cuts and flux; grazing / Wood-anomaly guards;
geometry and sampling validation; Fourier factorization; symmetry folding;
eigenmodes; Redheffer algebra; general (slant / out-of-plane) assembly;
convergence; tensor convolutions; and the homogeneous-mode cache.

Two blocks live in their own files because they are genuine LEAVES -- nothing
in them reads the rest of this module -- and ``_core`` re-exports both so every
existing import path resolves unchanged:

    ``_geometry.py``    analytic-shape support, overlap, unit-disk hits,
                        y-invariance; numpy only
    ``lumenairy/_branchcut.py``   the on-cut band comparison and the forward
                        selectors, shared with the EME, PMM and BOR engines
...
from ..._branchcut import band_mask as _band_mask, signed_forward as _signed_forward
from ...backend import (
    CUPY_AVAILABLE,
    JAX_AVAILABLE,
    array_namespace,
...
# The BLAS-thread-cap leaf: the opt-in cap on the BLAS pool for the NumPy /
# CuPy solves.  Re-exported (and listed in ``__all__``) so ``_core`` and
# ``lumenairy.elements.rcwa`` keep publishing these seven names as the SAME
# objects; the cap's mutable state stays in ``_blas`` -- see the note at the
# old site below.
from ._blas import (
    _BLAS_STATE,
    _blas_limit,
...
# The analytic-shape geometry-predicate leaf.  Imported here and listed in
# ``__all__`` so ``_core`` and ``lumenairy.elements.rcwa`` keep exporting these
# names; ``_validate_shapes``, which composes them, is defined below.
...
# The BLAS-thread cap lives in the ``_blas`` leaf (imported above with the
# other leaves).  Its module-level MUTABLE state -- ``_BLAS_CONTROLLER``,
# ``_BLAS_CONTROLLER_UNAVAILABLE``, ``_BLAS_WARNED_UNCONTROLLABLE`` -- and the
# three functions that read it at call time stay THERE and are deliberately not
# re-exported here, because that is where the code reads them: a test that
# substitutes one must patch ``rcwa._blas``, and a stale ``setattr`` on
# ``_core`` then raises AttributeError instead of binding a shadow attribute
# nothing reads.
...
def _sqrt_decay(x, xp=None, band: float = _CUT_BAND_REL):
    ...
    * ``conj`` IS NOT HOLOMORPHIC, so reverse-mode AD conjugates the cotangent
    ...
    # paragraph for the bound this buys.  The band comparison itself is the
    # shared one (:func:`lumenairy._branchcut.band_mask`), which the EME, PMM
    # and BOR engines call with their own scales; the scale above stays here
    # because it is this engine's (dimensionless, floored at 1.0).
    flip = (_band_mask(r.real, scale=scale, band=band, xp=xp) & (r.imag < 0)
            & (r.imag ** 2 > r.real ** 2))
    # The flip is ``-r``, NOT ``conj(r)``.  ``-r`` is the EXACT
    ...
    # cotangent.  See the docstring for both measurements, and
    # :func:`lumenairy._branchcut.signed_forward` for why the real-multiply
    # spelling is not interchangeable with ``where(flip, -r, r)`` on complex
    # input.
    return _signed_forward(r, flip, xp=xp)
...
# The analytic-shape GEOMETRY PREDICATES -- the support function, the exact
# overlap test, the unit-disk hit tests and the y-invariance test -- live in
# ``_geometry.py``, a leaf that imports only numpy.  They are imported at the
# top of this module and re-exported through ``__all__``, so every
# ``_core._shapes_overlap`` / ``rcwa._shape_support`` spelling still resolves
# here.  ``_validate_shapes`` below stays in this module because it is the
# POLICY that composes those predicates into the public refusals, and because
# it resolves ``_shapes_overlap`` through THIS module's globals -- which is
# what ``test_niche_audit_w9_overlap_exact.py``'s counting wrapper substitutes.
# ``_validate_cell_sampling`` above likewise stays with the Fourier machinery
# it guards.
        ...
        for d in dims:
            if not (np.isfinite(d) and d > 0.0):
                raise ValueError(
                    ...
                    f"be > 0 metres.")
        eps_sh = sh.get("eps")
        # ``np.complex128`` is ``_core._C``, the RCWA internal eigenproblem
        # dtype, spelled out here so this leaf imports nothing from ``_core``.
        if (eps_sh is not None and not callable(eps_sh)
                and abs(np.complex128(eps_sh)) < 1e-12):
            raise ValueError(
