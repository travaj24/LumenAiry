...
def apply_thin_lens(
    ...
    *,
    ...
    lens_model : str
        ...
        ``'nonparaxial'``
            ...
            The ``sign(f)`` factor is what makes ``f < 0`` diverge: the
            unsigned ``k*(f - sqrt(f**2+r**2))`` form converges for BOTH
            signs.  Exact for collimated->focus or focus->collimated; at
            FINITE conjugates it over-corrects (use ``'stigmatic'``).
        ``'aplanatic'``
            Stigmatic spherical phase restricted to the Abbe sine-
            condition domain r < |f| (unit phase outside).  Its quartic
            term is +k r**4/8f**3, the sign a converging sphere needs;
            the unsigned ``-k*f*(1 - sqrt(1 - r**2/f**2))`` profile
            carries the opposite one and DOUBLES paraxial's spherical
            aberration instead of removing it.
        ...
        ``'local_only'``
            **Deprecated since v5.29.1 (audit E-H7).**  It is a
            ...
            The NO-STEER model is the plain decentered
    ...
    ------
    ValueError
        If ``f`` is not finite.  A NaN or infinite focal length is not a
        lens, and left to the models they disagree completely about the
        damage: all-NaN under ``'paraxial'`` / ``'nonparaxial'`` for a NaN
        ``f`` but a silent no-op under ``'aplanatic'`` (whose ``r < |f|``
        domain test is False everywhere, so the unit-phase sentinel covers
        the grid), and the reverse split for ``f=+-inf``.  Nothing is raised
        by any of them, and
        ...
        place -- would propagate the poison into the caller's own object.
    ValueError
        If ``f == 0``.  Physically meaningless for a thin lens: an
        infinite-power surface with no realisation.  Unguarded, the five
        ``lens_model`` branches split three ways between a bare
        ``ZeroDivisionError('division by zero')`` naming neither this
        function nor the argument and returning ``E_in`` EXACTLY unchanged.
        The guard sits above the ``lens_model`` dispatch so all five raise
        identically, and :func:`apply_cylindrical_lens` carries the matching
        guard.  Nearby small-``f`` values are untouched bit-for-bit -- only
        exact zero is rejected.  The measured pre-guard tables are in
        docs/history/lumenairy.elements._lens_thin.md.
    ...
    * :func:`lumenairy.elements.thin_grating_efficiency_1d` -- an
    ...
    # A non-finite or zero ``f`` is REJECTED here, ABOVE the ``lens_model``
    # dispatch, so all five models answer the same unusable input the same way
    # (audit W4-1 / W5-2).  Left to the models they disagree completely: a NaN
    # ``f`` is an all-NaN field on 'paraxial'/'nonparaxial' and a SILENT NO-OP
    # on 'aplanatic' (its ``r**2/f**2 < 1`` domain test is False everywhere, so
    # the unit-phase sentinel wins); an infinite ``f`` is a flat no-op on
    # 'paraxial' and inf - inf on the two sqrt models; ``f = 0`` is a bare
    # ZeroDivisionError on three of them and a silent no-op on the other two.
    # The guard is also what protects ``JonesField.apply_thin_lens``
    # (polarization.py), which routes BOTH components through here and mutates
    # itself in place while returning ``self`` -- so a NaN ``f`` poisons the
    # caller's own object, every pixel of Ex AND Ey, with nothing raised.
    # Note ``f`` is NOT periodic, so there is nothing for the caller to reduce:
    # an infinite focal length means "no lens", which is what omitting the call
    # expresses; pass a large finite ``f`` if you want the residual power.  The
    # measured pre-guard tables are in
    # docs/history/lumenairy.elements._lens_thin.md.
    _f = float(f)
    ...
    if lens_model == 'paraxial':
        ...
        # SIGN-SAFE stigmatic sphere (thin-lens audit 2026-07-18, bug 1):
        # ``phi = -sign(f) * k * (sqrt(r**2 + f**2) - |f|)``.  Without the
        # ``sign(f)``, ``exp(1j*k*(f - sqrt(f**2 + r_sq)))`` expands for
        # f < 0 to a CONVERGING quadratic -k r**2/(2|f|) -- a diverging lens
        # that focuses identically to its +|f| twin.  For f > 0 the two are
        # byte-identical (IEEE negation of an exact subtraction).
        ...
        #
        # The phase is the exact stigmatic sphere restricted to the Abbe
        # sine-condition domain r < |f| (thin-lens audit 2026-07-18, bug 2).
        # It is NOT ``-k*f*(1 - sqrt(1 - r**2/f**2))``: that expands to
        # ``-k r**2/2f - k r**4/8f**3``, the WRONG quartic sign -- a
        # converging sphere needs ``+k r**4/8f**3`` -- so it carries 2x
        # paraxial's spherical-aberration error in the SAME direction.  The
        # sqrt(cos theta) pupil APODIZATION that distinguishes a true
        # aplanat is an AMPLITUDE factor a pure phase mask must not apply
        # (see docstring).
        ...
        # DEPRECATED (v5.29.1, audit E-H7).  The sum below expands to an
        # ORIGIN-centred parabola plus a constant piston, so the local
        # gradient at (xc, yc) is -k*xc/f and the sub-beam is steered ONTO
        # THE AXIS.  The zero-gradient (no-steer) model is the plain
        # decentered 'paraxial' lens.  Behaviour is deliberately unchanged
        # (0 callers in-repo, but it is a public enum value -- deprecate,
        # don't break); see the docstring for the algebra and the measured
        # -20.0 mrad steer.
...
def apply_spherical_lens(
    ...
    *,
    ...
    aperture_diameter : float or None
        ...
        come back **NaN** (v5.30, audit E-M8).  NaN is the
    ...
    * Orientation blindness.  The screen sees ``sag1 - sag2`` only, so
    ...
    def _surface_sag(h_sq, R):
        ...
        # CLAMPING ``h_sq`` instead (to ``0.9999 R**2``) saturates the sag at
        # a finite ``0.99 R`` and therefore imprints a unit-magnitude phase
        # screen for a surface that is not there: with ``aperture_diameter``
        # larger than ``2|R|`` those pixels survive the aperture -- measured
        # at R = 10 mm, aperture_diameter = 28 mm, N = 256, dx = 120 um,
        # 20916 out-of-domain pixels leaving with |E| = 1.000000 and a bogus
        # sag of 9.9 mm, where the aspheric sibling on the SAME geometry
        # returns NaN at exactly those 20916 pixels.  The
        # ``aperture_diameter=None`` branch below already zeroes
...
def apply_aspheric_lens(
    ...
    *,
    ...
    -----------------------------------------------
    Two DIFFERENT conics are involved and they are easy to conflate.  The
    ``-n**2`` conic is often quoted as a third-order-SA null for the CURVED
    FIRST surface of a plano-convex lens under collimated input: it is the
    wrong surface for a real lens AND the wrong value for this screen
    (adversarial audit 2026-07-25, finding E-C1).  Measured with an exact
    meridional Snell + eikonal trace
    ...
    * THIS SCREEN.  Within the thin-screen model the SA-minimising conic on
    ...
    # Reject ODD aspheric powers on BOTH surfaces up front (audit R-8 /
    # E-L7).  ``_aspheric_sag`` below evaluates ``coeff * h_sq ** (power //
    # 2)`` on its flat AND its curved branch, so an odd power silently floors
    # to the next-lower EVEN one and the screen imprints a DIFFERENT surface
    # with no diagnostic: unguarded, ``A1={5: 1e6}`` returns a field
    ...
    def _aspheric_sag(h_sq, R, k_conic, A_coeffs):
        ...
        # OUTSIDE-CONIC-DOMAIN pixels are NaN, so a downstream aperture mask
        # explicitly zeros them rather than the sag silently extrapolating a
        # 1e-12 floor into a near-singular value (1e6 m for typical optics)
        # outside the surface domain.  The gate is ``norm < 0.9999``, as in
        # every sibling (``lenses.surface_sag_general``,
        # ``lenses._conic_sag_xp``, ``elements.apply_mirror``'s inline sag,
        # ``raytrace.conic_sag``), and NOT ``denom_arg > 0`` (i.e.
        # ``norm < 1.0``): the two differ on the thin shell
        # ``0.9999 <= norm < 1.0``, where the conic denominator
        # ``1 + sqrt(denom_arg)`` is within 1e-2 of its vertical tangent and
        # the sag is numerically meaningless -- the siblings NaN it.
        # Measured (R = 10 mm sphere, N = 2048, dx = 10 um): 352 pixels land
        # in that shell, and ``surface_sag_general`` returns NaN for all 352.
        # The ``aperture_diameter=None`` branch below uses the matching
        # ``h_sq < max_h_sq * 0.9999`` cut, so the two halves of this
        # function agree on where the surface stops existing.
...
def apply_cylindrical_lens(
    ...
    *,
    ...
    ValueError
        ...
        (v5.32, audit W5-2).  Unguarded, ``f=nan`` returns a field whose
        every pixel is ``nan+nanj`` with nothing raised, ``f=+-inf``
        collapses ``k/(2f)`` to zero and silently applies NO lens, and
        ``f=0`` raises a bare ``ZeroDivisionError('division by zero')``
        naming neither this function nor the argument.  Both guards
    ...
    # The SIBLING of the ``apply_thin_lens`` non-finite / zero-``f`` guard --
    # the only other ``f``-taking entry point in this module, with the
    # identical failures (audit W4-2 / W5-2).  Unguarded, ``f=nan`` returns a
    # field whose every pixel is ``nan+nanj`` with nothing raised, ``f=+-inf``
    # collapses ``k/(2f)`` to zero and silently applies NO lens, and ``f=0``
    # raises a bare ``ZeroDivisionError('division by zero')`` naming neither
    # this function nor the argument (this function has no ``lens_model``
    # knob -- it is unconditionally the paraxial ``k/(2f) x**2`` form -- so
    # there is no silent-no-op arm here to unify away).  Guarded here, in the
    # same change as its sibling, so the two entry points answer an unusable
    # ``f`` identically: 'paraxial' ``apply_thin_lens`` and this function
    # compute the same quadratic, and they must not drift.
    _f = float(f)
    if not np.isfinite(_f) or _f == 0.0:
        _detail = (
            "f=0 is not a lens: it is an infinite-power surface with no "
...
def apply_grin_lens(
    ...
    *,
    ...
    thin_form : bool, default False
        ...
        reproduce the short-rod screen.
