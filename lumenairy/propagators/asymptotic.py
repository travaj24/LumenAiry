...
def _phi_v2_hessian_batch(fit: CanonicalPolyFit,
                           ...
                           v2x: np.ndarray, v2y: np.ndarray) -> np.ndarray:
    """Vectorised :func:`_phi_v2_hessian` over arrays of pixels.

    Returns an ``(N, 2, 2)`` real array where each ``[k]`` is the same
    Hessian that :func:`_phi_v2_hessian` would return for pixel ``k``.

    Implementation notes
    --------------------
    Uses the SAME term-by-term accumulation order as the scalar
    :func:`_phi_v2_hessian` (loop over multi-index, skip-if-zero) so the
...
def _compute_M_b_batch(fit: CanonicalPolyFit,
                        ...
                        v_cx: float, v_cy: float
                                   ...
                                   np.ndarray, np.ndarray, np.ndarray,
                                   np.ndarray]:
    """Vectorised :func:`_compute_M_b` over arrays of pixels.
...
def _gaussian_moment_table_2d_batch(M: np.ndarray, max_total_order: int
                                                ...
                                                np.ndarray]:
    """Vectorised :func:`gaussian_moment_table_2d` over an ``(N, 2, 2)``
    batch of M matrices.

    Returns ``(keys, table)`` where ``keys`` is the same ``(a, b)`` index
    enumeration that :func:`gaussian_moment_table_2d` produces (in
...
def _batched_polynomial_substitute_linear_2d(
    ...
    """Batched analogue of :func:`_polynomial_substitute_linear_2d`.
...
def _batched_polynomial_under_affine_shift(
    ...
    """Batched :func:`_polynomial_under_affine_shift`.
...
def _solve_envelope_stationary_batch(
    ...
    """Vectorised :func:`solve_envelope_stationary` over a 1-D batch of
    output pixels.
