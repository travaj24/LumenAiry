...
def test_stripe_fixture_is_free_of_the_mode_match_degeneracy():
    ...
    RE-DERIVED 2026-09-11 (branch-cut ROUND 3).  The version of this test that
    ...
    # ``worst`` records warnings itself, so the count it returns IS the
    # assertion -- a ``pytest.warns`` wrapper around it would see nothing,
    # because the inner ``catch_warnings`` has already consumed them.
    ...
    assert pre_warned >= 1, (
        "the pre-round-1 arm reopened the defect but the lossless-closure "
        "guard stayed silent on all %d truncations" % len(list(ladder)))
