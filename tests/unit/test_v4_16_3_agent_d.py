...
# ============================================================================
# audit closure: P3-NEW-F1-4 -- per-surface thickness in high-NA message
# ============================================================================
...
needs_jax = pytest.mark.skipif(
    not _JAX_AVAILABLE, reason="JAX is not installed")


_HIGH_NA_TRACE_PRESCRIPTION = {
    ...
    # Asymmetric thicknesses: surface 1 is the longest gap, so the
    # warning should cite surface index 1 (the worst per-surface
    # contributor), NOT just any of the surfaces.
    'thicknesses': [1.0e-3, 5.0e-3, 0.0],
}


@needs_jax
def test_high_na_warning_cites_surface_index_and_thickness():
    """audit closure: P3-NEW-F1-4

    The v4.16.2 hoist passed ``max(|thickness|)`` to the drift
    estimator, which cited the worst-gap thickness but not its
    surface index.  v4.16.3 cites the surface index explicitly so
    ...
    try:
        ...
        state = make_jax_ray_state(zeros, zeros, zeros, L, zeros, n_axis)
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter('always')
            _ = trace_jax(state, _HIGH_NA_TRACE_PRESCRIPTION,
                          wavelength=0.55e-6)
        rw = [w for w in caught
              if issubclass(w.category, RuntimeWarning)
              and '_transfer_jax' in str(w.message)]
        assert len(rw) == 1, (
            f"Expected exactly 1 high-NA RuntimeWarning; got "
            f"{len(rw)}: {[str(w.message) for w in rw]}.")
        msg = str(rw[0].message)
        # Cite the worst-case surface index.  Surface 1 has the
        # 5.0e-3 gap; surface 0 has 1.0e-3; surface 2 has 0.0.
        assert 'surface index 1' in msg, (
            f"High-NA warning must cite the worst-case surface "
            f"index (surface 1 = 5e-3 m gap); got: {msg!r}")
        # Cite the surface's own thickness, not max-of-all.  The
        # worst surface here is also the max, so 5e-3 is correct;
        # the regression we're guarding against is a "thickness="
        # field reporting a stale max from a different surface.
        assert '5.000e-03' in msg or '5e-03' in msg or '5.00e-03' in msg, (
            f"High-NA warning must cite the worst-case surface's "
            f"own thickness (5e-3 m); got: {msg!r}")
    finally:
        _reset_transfer_jax_warning()
        _TRACE_JAX_CACHE.clear()
