...
def test_vacuum_exact(n_orders):
    o, R, T, dof = pmm_efficiency_2d_staggered(
...
def test_position_invariance():
    ...
    o1, R1, T1, _ = pmm_efficiency_2d_staggered(eps_cell=_PILLAR, **base)
    pillar2 = np.array([[1.0, 1.0], [1.0, 2.25]], dtype=complex)  # shifted (1,1)
    o2, R2, T2, _ = pmm_efficiency_2d_staggered(eps_cell=pillar2, **base)
