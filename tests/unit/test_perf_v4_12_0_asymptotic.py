...
* **Perf #5 -- modal asymptotic LG polynomial hoist & cache.**
  ...
  ``lg_polynomial`` helper itself.  These tests pin:
    - the per-pixel results agree bit-near-exactly with a reference
      that bypasses the cache (rebuilds each pixel).
...
class TestPropagateModalAsymptoticCorrectness:
    ...
    def test_lg00_single_mode_matches_reference(self, fit, grid):
        ...
        ref = _scalar_pixel_reference_propagate_modal(
            fit,
            source_point=(0.0, 0.0),
            source_amplitudes=source_amps,
            pupil_amplitudes=pupil_amps,
        ...
        # Use relative tolerance 1e-12 -- the only difference is the
        # source of ``lg_polynomial`` (cached vs rebuilt); both run the
        # same algorithm so we expect bit-equal at peak magnitude.
        peak = float(np.max(np.abs(ref)))
        if peak == 0:
            pytest.skip("reference produced no signal")
        max_diff = float(np.max(np.abs(new - ref)))
        rel = max_diff / peak
        assert rel < 1e-10, (
            f"LG_(0,0) single-mode propagator drifted from reference: "
    ...
    def test_multimode_matches_reference(self, fit, grid):
        ...
        rel = max_diff / peak
        assert rel < 1e-10, (
            f"Multi-mode propagator drifted from reference: "
            f"max|new - ref| = {max_diff:.3e}, peak = {peak:.3e}, "
            f"rel = {rel:.3e}"
        )
